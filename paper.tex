...
Implementation details are covered in Section~\ref{sec:implementation-aspects}.
...
$
f_1\;op\;f_2 =
\overline{x} \cdot \left(f_1|_{x\leftarrow 0}\;op\;f_2|_{x\leftarrow 0}\right)
+ x \cdot \left(f_1|_{x\leftarrow 1}\;op\;f_2|_{x\leftarrow 1}\right)
$
\end{center}
...
step, \textit{Apply} generates a reduced graph immediately~\cite{BRYANT92}.

There are some refinements when it comes to implementation. Those are covered in
Section~\ref{sec:implementation-aspects} as well as the original paper of
Bryant~\cite{BRYANT86, BRYANT92}. \tbr

...
\noindent
For a given node $x$, the ITE operator $ITE(x, hi(x), lo(x))$ chooses $hi(x)$ if
$x$ evaluates to true and $lo(x)$ otherwise. Computing $ITE(x, hi(x), lo(x))$ is
accomplished by utilizing the following recursive form:
\begin{center}
$
ITE(x, hi(x), lo(x)) =
(x, ITE(), ITE())
...
efficient OBDD package implementation. \tbr

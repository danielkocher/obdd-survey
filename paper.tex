...
denotes the variable evaluates to 0 (left) or 1 (right), respectively. There exist
two \textit{terminal} nodes, denoted \textbf{0} (\textbf{False}) and \textbf{1}
(\textbf{True}), which represent the result of $f$. To evaluate $f$ for a given
variable assignment, one just follows the path for the respective variable values
and the terminal nodes one exists is the result.
\tbr
